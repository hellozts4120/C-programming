...
char s[100],input[100],output=0,mima[100],tempo[100];int n,k,music1,music2,music3,i,up,j,temp,temp1,u=-1,v=0,countwrong=0;
ifstream fin("1.txt");
printf("\t\t\t前次游戏记录\n");
 while(!(fin.eof())){
 fin>>s;                 
...
if(tempo[0]=='$'&&tempo[1]=='\0')
{
	...
	for(k=0,temp=1;play[0].password[k]!='\0';)
	{
		 if(mima[k]!=play[0].password[k]){
			printf("输入错误！请重新输入！\n");
			countwrong++;
			if(countwrong>3&&countwrong<=6){printf("错误次数过多，锁定3秒！\n");Sleep(3000);}
			if(countwrong>6&&countwrong<=9){printf("错误次数过多，锁定5秒！\n警告：再错误超过%d次后将自动退出！\n",10-countwrong);Sleep(5000);}
			if(countwrong==10){printf("未能成功验证，自动退出！\n");exit(0);}
			printf("请输入此用户名的密码：");
			gets_s(mima);
			k=0;}
...
{		play[i].round--;
		...
		system("pause");
		temp=rand()%256;
		SetConsoleTextAttribute(hConsole, temp);		//自创功能：此处为样例中的变色效果，原理为使用API设置控制台窗口字体颜色和背景色的函数，配合随机数的生成函数进行颜色变换。
	printf("游戏规则：输入屏幕随机出现的大写英文字母的小写值，游戏进行时任何时候输入“*”或“exit”将视为退出游戏！”\n");
printf("请输入你的名字：（输入“*”表示退出游戏）");
		n++;
		gets_s(play[n].name);
		...
		gets_s(tempo);
{
		if(tempo[0]=='1'&&tempo[1]=='\0')
			for(temp=0;temp<=i;temp++)
			{
				for(j=0;play[temp].password[j];j++){
					if(play[temp].password[j]>='0'&&play[temp].password[j]<='9'){play[temp].password[j]-=15;continue;}
					if(play[temp].password[j]>='A'&&play[temp].password[j]<='Z'){play[temp].password[j]+=32;continue;}
					if(play[temp].password[j]>='a'&&play[temp].password[j]<='z'){play[temp].password[j]-=32;continue;}
				}
				printf("%s\n",play[temp].password);
			}
		...
		for(j=0;j<=i;j++)
		{	
			fout<<"用户名："<<play[j].name<<"局数："<<play[j].round<<"正确数："<<play[j].countright<<"得分："<<play[j].score<<"总时间："<<play[j].timecount<<"加密后的密码："<<play[j].password<<"\n";
		}
		system("pause");
			exit(0);
	}
			...
			k=0;}
			 ...
			 temp++;
				...
				{	if(mima[k]!=play[u].password[k]){
					...
					countwrong=0;
					printf("输入正确！\n");
					printf("欢迎，%s！修改密码请输入“$”，查看此用户历史游戏记录请输入“#”,输入其他字符可开始游戏：",play[n].name);
						gets_s(tempo);
						if(tempo[0]=='#'&&tempo[1]=='\0')		//自创功能：利用strcmp函数实现查询已有用户的游戏信息。（之前的查询是使用循环实现的）
						{
							for(v=0;v<=i;v++)
								if(strcmp(play[v].password,play[n].password)==0)
								{
									cout<<"用户名："<<play[v].name<<"局数："<<play[v].round<<"正确数："<<play[v].countright<<"得分："<<play[v].score<<"总时间："<<play[v].timecount<<"\n";
								}
						}
						if(tempo[0]=='$'&&tempo[1]=='\0')
				{	
		 ...
		 if(play[n].password[temp]=='\0'&&mima[temp]!='\0'){
			...
			k=0;}
			 ...
			 temp++;
					...
					printf("请输入新的密码：");
					gets_s(play[n].password);
					strcpy_s(play[u].password,play[n].password);
						}
					system("pause");
					break;
			}
