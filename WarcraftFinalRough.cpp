    ...
    */  
    ...
    Warrior::Warrior( Headquarter * p,int no_,int kindNo_ ) {  
		...
		printf("%03d:00 %s %s %d born\n",nTime/60,color.c_str(),names[kindNo].c_str(),no);
			...
			while((nTime <= TimeEnd) && (!HeadOccupy)) {  
				switch (nTime % 60)
				{
					...
					case 10:
							...
							if(pCity[nCity] -> WarriorExist(0)){
								pCity[nCity] -> RedWarrior -> Move();
								BlueHead.EnemyComeIn(nTime,pCity[nCity] -> RedWarrior);
								pCity[nCity + 1] -> RedWarrior = pCity[nCity] -> RedWarrior;
								pCity[nCity] -> RedWarrior = 0;
								HeadOccupy = (BlueHead.Occupy(nTime) || HeadOccupy);
							}
